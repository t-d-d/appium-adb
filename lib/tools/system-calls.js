...
const DEFAULT_ADB_EXEC_TIMEOUT = 120000; // in milliseconds
...
systemCallMethods.waitForEmulatorReady = async function (timeoutMs = 60000) {
  let start = Date.now();
...
systemCallMethods.waitForDevice = async function (appDeviceReadyTimeout = 60) {
...
systemCallMethods.reboot = async function () {
  ...
  await retryInterval(180, 1000, async () => {
